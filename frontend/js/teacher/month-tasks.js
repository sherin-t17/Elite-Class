...
EC.teacherMonthTasks = {
  ...
  render(el) {
    el.innerHTML = `
      <div class="page-header">
        <div>
          <h2 class="page-title">🗓️ Month Tasks</h2>
      ...
      <div class="overlay" id="month-task-batch-modal">
        <div class="modal modal-lg">
          <div class="modal-header">
            ...
            <button class="modal-close" onclick="EC.teacherMonthTasks.closeBatchForm()">✕</button>
          </div>
      ...
      <div class="overlay" id="month-task-create-modal">
        <div class="modal modal-lg">
          <div class="modal-header">
            ...
            <button class="modal-close" onclick="EC.teacherMonthTasks.closeTaskForm()">✕</button>
          </div>
          <div class="modal-body">
      ...
      <div class="overlay" id="month-task-review-modal">
        <div class="modal">
          <div class="modal-header">
            ...
            <button class="modal-close" onclick="EC.teacherMonthTasks.closeReview()">✕</button>
          </div>
          <div class="modal-body">
  ...
  async saveBatchSettings() {
    ...
    try {
      await EC.api.updateMonthTaskBatch(this.state.currentBatch.id, {
  ...
  async saveTopPerformerOverride() {
    if (!this.state.currentBatch) return;
    try {
      await EC.api.setMonthTaskTopPerformerOverride(this.state.currentBatch.id, {
  ...
  async addTask() {
    ...
    try {
      ...
      await EC.api.createMonthTask(this.state.currentBatch.id, {
  ...
  async editTask(taskId) {
    ...
    if (!title) return;
    try {
      await EC.api.updateMonthTask(this.state.currentBatch.id, taskId, {
  ...
  async extendDeadline() {
    ...
    try {
      await EC.api.extendMonthTaskDeadline({
        ...
        studentId: document.getElementById('mt-extension-student')?.value,
      ...
      EC.toast('Deadline extended by 1 day.', 'success');
    } catch (err) {
  ...
  downloadTemplate() {
    const csv = [
      'Task Number,Title,Description,Difficulty,Marks/XP Value,Category,Needs Submission (Yes/No)',
      '1,LinkedIn Optimisation,Update profile banner and headline,Easy,10,Branding,No',
      '2,Portfolio PDF,Upload latest portfolio PDF,Medium,20,Portfolio,Yes'
    ].join('\n');
